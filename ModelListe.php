                ...
                <tbody>
                    ...
                    if ($ModelSayisi > 0) {
                        while ($satir = $ModelGetir->fetch(PDO::FETCH_ASSOC)) {
                            ?>
                            <tr>
                                <td data-title="Model Resmi"><img width="100" src="/ModelPic/<?= $satir['modelResmi'] ?>" alt=""
                                ...
                                <td data-title="Kesim Tarihi" class="align-middle">
                                    <button type="button" class="btn btn-dark"><?=$satir['kesimAdedi']?></button><br>
                                    <span style="font-size:10px"><?= $satir['kesimTuru'] ?> / adet</span><br>
                                    <span style="font-size:10px"><?=tarihCevir($satir['maliyetTarihi']) ?></span>
                                </td>


                                <td data-title="Kesim Adedi" class="align-middle"><?=getModelHareketleriIndex($satir['modelID'])?></td>


                                <td data-title="Maliyet Toplamı" class="align-middle">
                                    <button type="button" class="btn btn-primary"><?=rakam(getBirimMaliyet($satir['modelID']))?> ₺ / adet</button><br>
                                    <small>
                                        Toplam : <?php
